...
class Main:
    def __init__(self):
        ...
        cursor.hide()
        txt=self.format(" Kiro ヘ（。□°）ヘ ", " ID Video Tóp Tóp Của Bạn :  ")
        YourId=input(txt)
        self.videos = YourId
...
╚═╝  ╚═╝╚═╝╚═╝  ╚═╝ ╚═════╝ ╚═╝
        ...
        print(
            Colorate.Vertical(
                Colors.DynamicMIX((Col.red, Col.cyan)), Center.XCenter(txt)
            )
        )
    def title(self):
        if os.system != "nt":
        ...
        while True:
            ...
            try:
                ...
                os.system(
                    f"title Kiro ヘ（。□°）ヘ ^| Views: {views} ^| Elapsed Time: {curr_time} ^| v2.1"
                    if os.name == "nt"
                    else ""
                )
                time.sleep(0.5)
                ...
                os.system(
                    f"title Kiro ヘ（。□°）ヘ  ^| Views: Lỗi ^| Elapsed Time: {curr_time} ^| v2.1"
                    if os.name == "nt"
                    else ""
                )
                pass
    def solve_captcha(self, sessid):
        try:
            ...
            print(self.format(" Kiro :3 ",f" Solved captcha ! | {captcha_answer}"))
            return alpha_key
            ...
            print(
                self.format(
                    "!",
                    "Captcha Invalid | Kiểm tra truy cập từ Zefoy | Tik Tok : Kiro ",
                )
            )
            self.solve_captcha(sessid)
    def get_sessid(self):
        ...
        while True:
            try:
                ...
                if "This service is currently not working" in decryped_answer:
                    ...
                    print(self.format(" Kiro ヘ（。□°）ヘ  ",f" Delay: {timer}     "), end="\r")
                    start = time.time()
                    while time.time() < start + timer:
                        print("\r", end="")
                        print(self.format(" Kiro ヘ（。□°）ヘ  ", f" Delay: {round((start + timer) - time.time())}       "),end="\r")
                        time.sleep(1)
                    continue
                ...
                if latency > 3:
                    print(self.format(" Kiro ヘ（。□°）ヘ  ",f" Gửi views ID: {aweme_id}"))
                ...
                print(self.format(" Kiro ヘ（。□°）ヘ  ",f" Còn: {timer}    "), end="\r")
                start = time.time()
                while time.time() < start + timer:
                    print("\r", end="")
                    print(self.format(" Kiro ヘ（。□°）ヘ  ",f" Còn: {round((start + timer) - time.time())}     "), end="\r")
                    time.sleep(1)
            except:
                os.system("python " + sys.argv[0])
                sys.exit(0)
...
class Menu:
    def format(self, symbol, text):
        self.blue = Col.red
        self.lblue = Colors.StaticMIX((Col.red, Col.white, Col.white))
        return f"""   {Col.Symbol(symbol, self.lblue, self.blue)} {self.lblue}{text}{Col.reset}"""
    def main(self):
        banner=requests.get("https://pastebin.com/raw/JrRVuYVX").text
